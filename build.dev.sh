...
BUILD_PUBLIC_ZIP_FILE_PATH="${BUILD_DEPLOY_RELATIVE_PATH}/${BUILD_PUBLIC_ZIP_FILE}"
...
# Create ZIP file
echo "---- ZIPPING SplMap BUILD TO ${BUILD_DEPLOY_RELATIVE_PATH}/${BUILD_PUBLIC_ZIP_FILE} ----"
rm -rf ${BUILD_PUBLIC_ZIP_FILE_PATH}
(cd ${BUILD_DEPLOY_RELATIVE_PATH} && ${ZIP_CMD} a -r ${BUILD_PUBLIC_ZIP_FILE} ${BUILD_PUBLIC_DIR} > /dev/null)
